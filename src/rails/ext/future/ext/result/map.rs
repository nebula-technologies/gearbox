...
impl<Fut, FutFunc, FuncOutput, T, U, E> Map<Fut, FutFunc, FuncOutput>
where
...
impl<Fut, FutFunc, FuncOutput, T, U, E> Future for Map<Fut, FutFunc, FuncOutput>
where
    Fut: Future<Output = Result<T, E>>,
    FutFunc: FnOnce(T) -> FuncOutput,
    FuncOutput: Future<Output = U>,
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        match this.state {
            ...
            MultiState::Processing { ref mut future } => {
                unsafe { Pin::new_unchecked(future) }.poll(cx).map(Ok)
            }
